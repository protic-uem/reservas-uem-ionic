...
import {
  disciplinasPorUsuario,
  disciplinasPorDepartamento
} from "../../graphql/disciplina/disciplina-json";
...
export class DisciplinaServiceProvider extends ConexaoProvider {
  ...
  async carregarDisciplinasPorDepartamento(id_departamento: number) {
    ...
    return await new Promise((resolve, reject) => {
      this.http
        .post(this.baseUri, disciplinasPorDepartamento(id_departamento), {
  ...
  async carregarDisciplinasPorUsuario(id_usuario: number) {
    ...
    return await new Promise((resolve, reject) => {
      this.http
        .post(this.baseUri, disciplinasPorUsuario(id_usuario), {
          headers: ConexaoProvider.headersToken
        })
        .subscribe((result: any) => {
          if (result.errors) {
