...
export class ReservaCreateSearchPage {
  ...
  constructor(
    ...
    if (this.login.nome == undefined) this.loadResources();
    else {
    ...
    this.disciplinaService
      ...
      .then((disciplinas: Array<DisciplinaGraphql>) => {
        if (disciplinas.length > 0) {
          ...
          apresentarErro(
            this.alertCtrl,
            "Nenhuma disciplina foi encontrada para reste usuário"
          );
        }
      })
      .catch(error => {
  ...
  changeUsuario(usuario) {
    if (usuario != undefined) this.carregarDisciplinasPorUsuario(usuario.id);
  }
  ...
  reservaConfirm() {
    ...
    if (
      ...
      msg =
        ...
        "<br/>" +
        "<b>Data reservada:</b> " +
        format(this.reserva.data_reserva, "DD/MM/YYYY") +
        "<br/>" +
