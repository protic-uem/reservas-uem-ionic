...
export class LoginPage {
  ...
  loginAutomatico() {
    ...
    loading.present();
    this.loginService
      .confirmLogin(this.email.toLowerCase().trim(), this.senha.trim())
      .then((usuario: UsuarioGraphql) => {
        ...
        this.carregarSalasPorDepartamento(
          this.usuario.departamento.id,
          loading
        );
      })
      .catch(error => {
  ...
  atualizarMinhasReservas(loading: any) {
    this.reservaService
      .carregarMinhasReservas(this.usuario.id)
