...
export class ReservaSearchPage {
  ...
  constructor(
    ...
    if (this.login.nome != undefined) {
      ...
      else this.dataDocente = format(addMonths(new Date(), 1), "YYYY-MM-DD");
    }
  ...
  cadastrarReserva(periodo: number) {
    ...
    if (prosseguir) {
      ...
      apresentarErro(
        this.alertCtrl,
        "Não é permitido reservas no sábado ou domingo."
      );
    }
  }
  ...
  openCalendar() {
    ...
    myCalendar.onDidDismiss((date: CalendarResult, type: string) => {
      if (date != null && date != undefined) {
        ...
        this.carregarReservasPorDataSala(
          this.dataSelecionada,
          this.login.departamento.id,
          this.salaSelecionada.id
        );
      }
    });
  }
  ...
  carregarSalasPorDepartamento(id_departamento: number) {
    ...
    loading.present();
    this.storage
      .get("salasDepartamento")
      .then((salas: Array<SalaGraphql>) => {
        if (salas.length > 0) {
          this.salas = salas;
          this.cdr.detectChanges();
          this.selectRef.open();
        } else this.salas = new Array<SalaGraphql>();
        loading.dismiss();
      })
      .catch(error => {
        loading.dismiss();
  ...
  proximoDia() {
    if (!isAfter(this.dataSelecionada, this.dataDocente))
      this.dataSelecionada = format(
        addDays(parse(this.dataSelecionada), 1),
        "YYYY-MM-DD"
      );
  ...
  anteriorDia() {
    if (!isBefore(this.dataSelecionada, new Date()))
      this.dataSelecionada = format(
        subDays(parse(this.dataSelecionada), 1),
        "YYYY-MM-DD"
      );
